...
用戶：龜背芋葉子有黑斑？
蕨積：這是葉斑病，通常由濕度過高、通風不良引起。請剪除所有病葉，減少葉面噴水，確保環境通風。若持續惡化，可用稀釋銅劑噴灑，每週一次。

用戶：這是什麼植物？（附圖）
蕨積：根據葉片特徵，這是龜背芋（Monstera deliciosa），天南星科蔓綠絨屬。原產中美洲熱帶雨林，特色是成熟葉片會產生羽狀裂口和孔洞。
...
def ask_deepseek(question, user_name=None, is_professional=False):
    ...
    if is_professional:
        ...
        # 在問題前面加上強制前綴
        forced_question = f"""【重要】你現在是植物學博士，請用極度專業、冷靜、準確的方式回答以下問題。禁止使用任何語氣詞、表情符號、開玩笑。回答必須包含原因、解法、預防。
...
問題：{question}"""
        ...
        data = {
            ...
            "max_tokens": 400,          # 專業模式可以長一點
            "temperature": 0.1,          # 溫度降到最低，幾乎不發揮
            "top_p": 0.1                 # 極度保守，只選最可能的詞
        }
        
        print(f"🔬 專業模式 activated - 問題: {question[:30]}...")
        ...
        print(f"😊 賣萌模式 activated - 問題: {question[:30]}...")
...
def handle_text_message(event):
    ...
    print(f"📝 用戶 {user_id} | {mode} | 問題: {user_message[:30]}...")
    
    # 呼叫DeepSeek（強制專業模式已內建）
...
def health():
    ...
    return f"🌿 蕨積6.0 強制專業版 | Supabase: {supabase_status} | 排程器: {scheduler_status}", 200
